...
Power BI's `.pbix` format is binary and cannot be generated by script. The `powerbi/`
folder provides **all DAX measures** and a **step-by-step rebuild guide** (~20 min) from the
cleaned CSV. Save the result as `powerbi/Financial_Analysis.pbix`.
